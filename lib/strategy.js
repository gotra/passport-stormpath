...
function Strategy(o) {
  ...
  this._accessTokenField = opts.accessTokenField || 'accessToken';
  this._providerField = opts.providerField || 'provider' ;
  ...
  self.deserializeUser = function(userHref, done) {
    self.spClient.getAccount(userHref, {expand: 'customData'}, function(err,account) {
      done(err, account);
    });
  };
...
Strategy.prototype.authenticate = function(req, options) {
  ...
  var accessToken = helpers.lookup(req.body,this._accessTokenField) || helpers.lookup(req.query, this._accessTokenField);
  var provider = helpers.lookup(req.body,this._providerField) || helpers.lookup(req.query,this._providerField);


  if ((!username || !password) && (!accessToken || !provider))  {
